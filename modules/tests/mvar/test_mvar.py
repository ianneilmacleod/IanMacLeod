...
gxpy = os.environ['GEOSOFTPY']
if len(gxpy.strip()) == 0:
    print(
        "GEOSOFTPY environment is not defined.  This should point to the folder that contains Geosoft Python modules.")
    exit(0)
sys.path.insert(0, gxpy)
import gxpy.system as gsys
import gxpy.gx as gxp
import gxpy.gdb as gxgdb
import gxpy.mvar as mvar
...
class TestSOM(unittest.TestCase):
    ...
    def test_gdb(self):
        ...
        gdb = gxgdb.GXdb.new(self.gxPy, 'testdata\\gdbSOMtest.gdb')
        ...
        for l in range(image_in.shape[0]):
            gdb.writeDataLine('L{}'.format(l), image_in[l, :, :], channels=['R', 'G', 'B'])

        # select first line only
        gdb.selectLines('', True)
        # gdb.selectLines('L0',True)
    ...
    def test_big_gdb(self):
        ...
        gdb = gxgdb.GXdb.open(self.gxPy, 'E:\\Projects\\Simandou\\SimSouth200_Model.gdb')
    ...
    def test_gdb2(self):
        ...
        gdb = gxgdb.GXdb.open(self.gxPy, 'testdata\\gdbSOMtest2.gdb')
        gdb.selectLines('', True)
        # gdb.selectLines('L0',True)
