...
class SOMgdb:
    ...
    def __init__(self, gdb, fields, dim=16, per=2.0, normalize=NormType.none,
        ...
        self.progress = progress
        self.progress("Database: {}".format(gdb.fileName))
    ...
    def _readdata(self):
        ...
        # read line at a time
        lines = self.gdb.lines(select=True)
        nl = len(lines)
        ...
        for l in lines:
            ...
            npd, ch, fid = self.gdb.readLine(l, channels=self.fields, dummy=gxgdb.READ_REMOVE_DUMMYROWS)
    ...
    def _classify_db(self):
        ...
        self.gdb.delChan(self.outClass)
        self.gdb.newChannel(self.outClass, np.int32)
        self.gdb.delChan(self.outDist)
        self.gdb.newChannel(self.outDist, np.float64)
        ...
        for l in lines:
            ...
            data, ch, fid = self.gdb.readLine(l, channels=self.fields)
            ...
            self.gdb.writeDataChan(lsymb, self.outClass, clss)
            self.gdb.writeDataChan(lsymb, self.outDist, err)
